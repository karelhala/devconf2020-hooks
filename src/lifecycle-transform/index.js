...
const calculateOpened = (opened, breed, isOpen) => ({
  ...opened,
  [breed]: isOpen === undefined ? !opened[breed] : isOpen
})
...
const LifeCycleTransform = () => {
  ...
  const handleMenuOpen = (breed, isOpen) => setOpened((opened) => calculateOpened(opened, breed, isOpen));

  useEffect(() => {
    const { breed } = calculateBreed(location.search);
    breed && handleMenuOpen(breed, true);
    fetch('https://dog.ceo/api/breeds/list/all')
    .then(data => data.json())
    .then(({ message }) => Object.entries(message)
